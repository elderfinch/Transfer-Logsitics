...
let state = loadFromLS() || {
  ...
  lang: "pt",
  columnSettings: JSON.parse(JSON.stringify(defaults.columnSettings)),
};
let fileOldRows = null;
let fileNewRows = null;
let mapping = {
  name: null,
  type: null,
  area: null,
...
function determineCity(loc) {
  if (loc.area && state.exceptions[loc.area])
    return state.exceptions[loc.area].city;
  if (loc.district && state.exceptions[loc.district])
    return state.exceptions[loc.district].city;
  if (loc.zone && state.exceptions[loc.zone])
    return state.exceptions[loc.zone].city;
...
document.getElementById("btn-process").addEventListener("click", async () => {
  ...
  if (!fOld || !fNew) {
    M.toast({ html: translations[state.lang].toast_select_files });
    return;
  }
...
function showMappingModal(sampleRow) {
  const container = document.getElementById("mapping-forms");
  container.innerHTML = "";
  const keys = Object.keys(sampleRow || {});
  const fields = [
    { id: "name", label: "Name (Last, First)" },
    { id: "type", label: "Type (Elder/Sister)" },
    { id: "area", label: "Area/Neighborhood" },
...
document.getElementById("btn-map-apply").addEventListener("click", () => {
  ["name", "type", "area", "district", "zone", "companion"].forEach(
    (f) => (mapping[f] = document.getElementById("map-" + f).value || null),
  );
...
function prepareExceptionsAssignment() {
  ...
  [fileOldRows, fileNewRows].forEach((rows) => {
    (rows || []).forEach((r) => {
      if (mapping.zone && r[mapping.zone])
        allUnique.zones.add(r[mapping.zone].toString().trim());
      if (mapping.district && r[mapping.district])
        allUnique.districts.add(r[mapping.district].toString().trim());
      if (mapping.area && r[mapping.area])
        allUnique.areas.add(r[mapping.area].toString().trim());
    });
  });
  const container = document.getElementById("exceptions-assign");
  container.innerHTML = "";
  ...
  createSection("Area Overrides", allUnique.areas);
  createSection("District Overrides", allUnique.districts);
  createSection("Zone Overrides", allUnique.zones);
  M.AutoInit();
  M.Modal.getInstance(document.getElementById("modal-exceptions")).open();
}

document
  .getElementById("btn-exceptions-apply")
  .addEventListener("click", () => {
    document.querySelectorAll(".ex-apply").forEach((chk) => {
      const key = chk.dataset.key;
      const city = document
        .querySelector(`.ex-city[data-key="${key}"]`)
        .value.trim();
      if (chk.checked && city) {
        state.exceptions[key] = { city: city };
      } else if (state.exceptions[key] && !defaults.exceptions[key]) {
...
function prepareAndShowTransportModal() {
  ...
  (fileNewRows || []).forEach((r) => {
    ...
    const baseMissionary = {
      id,
      name: nameVal,
      lastName: nameParts[0] || "",
      firstName: nameParts[1] || "",
      type: /^(elder|sister)$/i.test((r[mapping.type] || "").toString().trim())
        ? (r[mapping.type] || "").toString().trim()
        : "",
      date: "",
      time: "",
      instructions: "",
      leader: false,
      companion:
        mapping.companion && r[mapping.companion]
          ? r[mapping.companion].toString().trim()
          : "",
    };
    if (old) {
      const oldLoc = {
      ...
      if (oldLoc.area !== newLoc.area) {
        // Simplified transfer condition
        pendingMissionaries.push({
          ...baseMissionary,
          originCity: determineCity(oldLoc),
          originArea: oldLoc.area,
          originDistrict: oldLoc.district,
          originZone: oldLoc.zone,
          destinationCity: determineCity(newLoc),
          destinationArea: newLoc.area,
          destinationDistrict: newLoc.district,
          destinationZone: newLoc.zone,
          isNew: false,
        });
      }
    } else {
  ...
  Array.from(uniqueRoutes)
    ...
    .forEach((route) => {
      ...
      container.appendChild(row);
      row.querySelector("select").value = defaultTransportForPair(
        origin,
        destination,
      );
    });
  M.Modal.getInstance(
    document.getElementById("modal-transport-defaults"),
  ).open();
}
...
document.getElementById("btn-transport-apply").addEventListener("click", () => {
  ...
  saveToLS();
  M.toast({
    html: translations[state.lang].toast_processed.replace(
      "{count}",
      pendingMissionaries.length,
    ),
  });
  location.reload();
});
...
function defaultTransportForPair(from, to) {
  const f = (from || "").toLowerCase(),
    t = (to || "").toLowerCase();
  if (
    (f === "nampula" && t === "quelimane") ||
    (t === "nampula" && f === "quelimane")
  )
    return "Bus";
...
function renderGroupsByCity() {
  const container = document.getElementById("groups-by-city");
  container.innerHTML = "";
...
function renderGroupsByTransport() {
  const container = document.getElementById("groups-by-transport");
  container.innerHTML = "";
  const byTransport = {};
  Object.values(state.groups || {})
    ...
    .forEach((m) => {
      ...
      byTransport[transport].push(m);
    });
  Object.keys(byTransport)
    .sort()
    .forEach((key) => {
      const card = document.createElement("div");
      card.className = "card group-card";
      ...
      const T = translations[lang];
      const transportName =
        T[`transport_${key.toLowerCase().replace("/", "_")}`] || key;
      content.innerHTML = `<span class="card-title">${transportName}</span>`;
      content.appendChild(
        createMissionaryTable(byTransport[key], "transport", key),
      );
      card.appendChild(content);
      container.appendChild(card);
    });
...
function renderMasterTable() {
  const container = document.getElementById("groups-master");
  container.innerHTML = "";
...
function createMissionaryTable(missionaries, groupType, groupKey) {
  ...
  table.className = "striped table-col-toggle";
  const lang = state.lang || "pt";
  const T = translations[lang];
  const sortedMissionaries = [...missionaries].sort(
    (a, b) =>
      a.isNew - b.isNew ||
  ...
  const bulkEditRow = allowBulkEdit
  ...
  sortedMissionaries.forEach((m) => {
    ...
    tr.dataset.id = m.id;
    let typeCellContent =
      m.type === "Elder" || m.type === "Sister"
        ? `<span>${m.type}</span>`
        : `<select class="type-select-editable browser-default"><option value="" selected disabled>${T.choose_option}</option><option value="Elder">Elder</option><option value="Sister">Sister</option></select>`;
    ...
    instrTextarea.addEventListener("input", () =>
      autoResizeTextarea(instrTextarea),
    );
      ...
      el.addEventListener("change", (e) => {
        ...
        if (missionary) {
          missionary.transport =
            trElement.querySelector(".transport-select").value;
          missionary.date = trElement.querySelector(".date-input").value;
          missionary.time = trElement.querySelector(".time-input").value;
          missionary.instructions =
            trElement.querySelector(".instr-input").value;
          missionary.leader =
            trElement.querySelector(".leader-checkbox").checked;
          if (el.classList.contains("type-select-editable")) {
            missionary.type = el.value;
          }
          saveToLS();
    ...
    tr.querySelector(".remove-btn").addEventListener("click", (e) => {
      if (confirm(T.confirm_remove_missionary)) {
        ...
        if (groupKey) {
          const indexToRemove = state.groups[groupKey].findIndex(
            (miss) => miss.id === missionaryId,
          );
          if (indexToRemove > -1) {
            state.groups[groupKey].splice(indexToRemove, 1);
    ...
    tr.querySelector(".edit-btn").addEventListener("click", (e) =>
      openEditModal(e.currentTarget.closest("tr").dataset.id),
    );
  });
  ...
  if (allowBulkEdit && groupKey) {
    const bulkUpdate = (field, value) => {
      ...
      if (groupType === "city") {
        ...
        missionariesToUpdate = Object.values(state.groups)
          .flat()
          .filter((m) => (m.transport || "Unassigned") === groupKey);
      }
      missionariesToUpdate.forEach((m) => {
        m[field] = value;
      });
      saveToLS();
      renderAllViews();
    ...
    if (bulkInstr) {
      bulkInstr.addEventListener("input", () => autoResizeTextarea(bulkInstr));
      bulkInstr.addEventListener("change", (e) =>
        bulkUpdate("instructions", e.target.value),
      );
    }
    table
      .querySelector(".bulk-transport-select")
      .addEventListener("change", (e) =>
        bulkUpdate("transport", e.target.value),
      );
    table
      .querySelector(".bulk-date-input")
      .addEventListener("change", (e) => bulkUpdate("date", e.target.value));
    table
      .querySelector(".bulk-time-input")
      .addEventListener("change", (e) => bulkUpdate("time", e.target.value));
  }
  return table;
}
...
function populateCityDropdowns() {
  const cities = new Set(
    Object.values(state.groups || {})
      .flat()
      .flatMap((m) => [m.originCity, m.destinationCity]),
  );
  const sortedCities = Array.from(cities).sort();
  const lang = state.lang || "pt";
  const selects = document.querySelectorAll(
    "#add-origin-select, #add-dest-select, #edit-origin-select, #edit-dest-select",
  );
...
function openEditModal(missionaryId) {
  ...
  const setupSelect = (type, city) => {
    const select = document.getElementById(`edit-${type}-select`),
      other = document.getElementById(`edit-${type}-other`);
    if (Array.from(select.options).some((o) => o.value === city)) {
      select.value = city;
  ...
  setupSelect("origin", missionary.originCity);
  setupSelect("dest", missionary.destinationCity);
  document.getElementById("edit-originarea").value =
    missionary.originArea || "";
  ...
  document.getElementById("edit-companion").value = missionary.companion;
...
const ALL_COLUMNS = [
  { key: "type", labelKey: "table_header_type" },
  { key: "lastName", labelKey: "table_header_last_name" },
  { key: "firstName", labelKey: "table_header_first_name" },
  { key: "originCity", labelKey: "origin_city" },
  { key: "destCity", labelKey: "dest_city" },
  { key: "originArea", labelKey: "table_header_origin_area" },
  { key: "destArea", labelKey: "table_header_dest_area" },
  { key: "companion", labelKey: "table_header_companion" },
  { key: "transport", labelKey: "table_header_transport" },
  { key: "date", labelKey: "table_header_date" },
  { key: "time", labelKey: "table_header_time" },
  { key: "instructions", labelKey: "table_header_instructions" },
  { key: "new", labelKey: "table_header_new" },
  { key: "leader", labelKey: "table_header_leader" },
];
...
document
  .getElementById("column-manager-wrapper")
  .addEventListener("click", (e) => {
    if (e.target.classList.contains("visibility-toggle")) {
      const chip = e.target.closest(".col-chip");
      const targetContainer =
        chip.parentElement.id === "visible-cols-container"
          ? document.getElementById("hidden-cols-container")
          : document.getElementById("visible-cols-container");
...
function updateColumnState() {
  const view = getActiveView();
  const visibleContainer = document.getElementById("visible-cols-container");
  state.columnSettings[view] = [...visibleContainer.children].map(
    ...
    table.querySelectorAll("thead tr, tbody tr").forEach((row) => {
      const cells = new Map(
        [...row.children].map((cell) => {
          const key = [...cell.classList]
            .find((c) => c.startsWith("col-"))
            .replace("col-", "");
          return [key, cell];
        }),
      );
...
["add", "edit"].forEach((prefix) => {
  document
    .getElementById(`${prefix}-origin-select`)
    .addEventListener("change", (e) => {
      document.getElementById(`${prefix}-origin-other`).style.display =
        e.target.value === "other" ? "block" : "none";
    });
  document
    .getElementById(`${prefix}-dest-select`)
    .addEventListener("change", (e) => {
      document.getElementById(`${prefix}-dest-other`).style.display =
        e.target.value === "other" ? "block" : "none";
    });
});
...
document.getElementById("btn-add-save").addEventListener("click", () => {
  const originCity =
    document.getElementById("add-origin-select").value === "other"
      ? document.getElementById("add-origin-other").value
      : document.getElementById("add-origin-select").value;
  const destCity =
    document.getElementById("add-dest-select").value === "other"
      ? document.getElementById("add-dest-other").value
      : document.getElementById("add-dest-select").value;
  ...
  const m = {
    ...
    originCity: normalizeZoneName(originCity),
    destinationCity: normalizeZoneName(destCity),
    originArea: document.getElementById("add-originarea").value,
    destinationArea: document.getElementById("add-destarea").value,
    transport: document.getElementById("add-transport").value,
    companion: document.getElementById("add-companion").value,
  ...
  state.groups[key].push(m);
  saveToLS();
  M.toast({ html: translations[state.lang].toast_added });
  location.reload();
});
...
document.getElementById("btn-edit-save").addEventListener("click", () => {
  ...
  if (!missionary) return;
  const originCity =
    document.getElementById("edit-origin-select").value === "other"
      ? document.getElementById("edit-origin-other").value
      : document.getElementById("edit-origin-select").value;
  const destCity =
    document.getElementById("edit-dest-select").value === "other"
      ? document.getElementById("edit-dest-other").value
      : document.getElementById("edit-dest-select").value;
  ...
  Object.assign(missionary, {
    ...
    originCity: normalizeZoneName(originCity),
    destinationCity: normalizeZoneName(destCity),
    originArea: document.getElementById("edit-originarea").value,
    destinationArea: document.getElementById("edit-destarea").value,
    transport: document.getElementById("edit-transport").value,
    companion: document.getElementById("edit-companion").value,
...
document.getElementById("btn-save-exception").addEventListener("click", () => {
  const name = document.getElementById("ex-add-name").value.trim(),
    city = document.getElementById("ex-add-city").value.trim();
  if (name && city) {
    if (!state.exceptions) state.exceptions = {};
    state.exceptions[name] = { city };
    saveToLS();
    renderExceptions();
    document.getElementById("ex-add-name").value = "";
    document.getElementById("ex-add-city").value = "";
    M.updateTextFields();
  }
});
...
function renderExceptions() {
  ...
  el.innerHTML = "";
  Object.keys(state.exceptions || {}).forEach((k) => {
    const row = document.createElement("div");
    row.className = "row";
    row.innerHTML = `<div class="col s4">${k}</div><div class="col s4">${state.exceptions[k].city || ""}</div><div class="col s4"><a data-key="${k}" class="btn-small red btn-ex-remove">Remove</a></div>`;
    el.appendChild(row);
    row.querySelector(".btn-ex-remove").addEventListener("click", (e) => {
      delete state.exceptions[e.currentTarget.dataset.key];
      saveToLS();
      renderExceptions();
    });
  });
...
document.getElementById("btn-clear-local").addEventListener("click", () => {
  if (confirm(translations[state.lang].confirm_clear_local)) {
...
document.getElementById("btn-restore").addEventListener("click", async () => {
  ...
  if (!file) {
    M.toast({ html: translations[state.lang].toast_restore_file });
    return;
  }
  ...
  for (const row of rows) {
    ...
    const transportVal =
      Object.keys(translations.en).find(
        (k) => k.startsWith("transport_") && translations.en[k] === cols[8],
      ) ||
      Object.keys(translations.pt).find(
        (k) => k.startsWith("transport_") && translations.pt[k] === cols[8],
      );
    const missionary = {
      lastName: cols[0],
      firstName: cols[1],
      type: cols[2],
      originCity: cols[3],
      destinationCity: cols[4],
      originArea: cols[5],
      destinationArea: cols[6],
      companion: cols[7],
      transport: (transportVal || "").replace("transport_", ""),
      date: cols[9],
      time: cols[10],
      instructions: cols[11],
      isNew: cols[12]?.toLowerCase() === "yes",
      leader: cols[13]?.toLowerCase() === "yes",
      name: `${cols[0]}, ${cols[1]}`,
      id: `${cols[0]}, ${cols[1]}`.toLowerCase(),
    };
  ...
  saveToLS();
  M.toast({ html: translations[state.lang].toast_restored });
  location.reload();
});
...
function createCalendarDescription() {
  ...
  const selectedIds = new Set(
    Array.from(
      document.querySelectorAll(
        '#missionaries-checkbox-list input[type="checkbox"]:checked',
      ),
    ).map((chk) => chk.value),
  );
  ...
  const destCities = [
    ...new Set(selectedMissionaries.map((m) => m.destinationCity)),
  ].join(", ");
  ...
  selectedMissionaries.forEach((m) => {
    ...
    const firstInitial = (m.firstName || "")[0]
      ? (m.firstName || "")[0].toUpperCase()
      : "";
    const shortName =
      `${typeInitial} ${firstInitial} ${m.lastName || ""}`.trim();
...
document.querySelector("body").addEventListener("click", (e) => {
  if (e.target.closest(".calendar-btn")) {
    ...
    if (groupType === "city" && state.groups[currentGroupKey]) {
      ...
      currentMissionaryGroup = Object.values(state.groups)
        .flat()
        .filter((m) => (m.transport || "Unassigned") === currentGroupKey);
    } else {
      // Master List (use a common attribute like date to group)
      ...
      if (m && m.date) {
        ...
        currentMissionaryGroup = Object.values(state.groups)
          ...
          .filter(
            (miss) =>
              miss.date === m.date && (miss.time || "") === (m.time || ""),
          );
      } else {
    ...
    if (groupType === "city") {
      ...
      defaultTitle = String(currentGroupKey || defaultTitle).replace(
        /\s*→\s*/g,
        " -> ",
      );
    } else if (groupType === "transport") {
      defaultTitle =
        T[
          `transport_${String(currentGroupKey || "")
            .toLowerCase()
            .replace("/", "_")}`
        ] ||
        currentGroupKey ||
    ...
    document.getElementById("calendar-event-date").value = refMissionary
      ? refMissionary.date || ""
      : "";
    document.getElementById("calendar-event-time").value = refMissionary
      ? refMissionary.time || ""
      : "";

    // Populate checkbox list using the current table order (same sorting as the displayed tables)
    let listToShow =
      currentMissionaryGroup && currentMissionaryGroup.length
    ...
    listToShow.forEach((m) => {
      ...
      const transportName = m.transport
        ? translations[state.lang][
            `transport_${(m.transport || "").toLowerCase().replace("/", "_")}`
          ] || m.transport
        : "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  state.exceptions = Object.assign(
    {},
    defaults.exceptions,
    state.exceptions || {},
  );
  if (!state.columnSettings) {
    state.columnSettings = JSON.parse(JSON.stringify(defaults.columnSettings));
  }
  Object.values(state.groups || {})
    .flat()
    .forEach((m) => delete m.tbd); // Clean up old data
  saveToLS();
  M.AutoInit();
  loadState();
});

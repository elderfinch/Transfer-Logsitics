...
document.getElementById("btn-export-pdf").addEventListener("click", () => {
  ...
  const autoTableOptions = {
    ...
    columnStyles: (function () {
      const cs = {};
      checkboxColIndices.forEach((i) => {
        cs[i] = { halign: "center", cellWidth: 14 };
      });
      return cs;
    })(),
    didDrawCell: function (data) {
      ...
      let boxSize = Math.min(data.cell.width, data.cell.height) * 0.35; // relative (smaller)
  ...
  if (view === "city") {
    ...
    Object.keys(groupsToPrint)
      ...
      .forEach((groupKey) => {
        ...
        doc.setFontSize(14);
        const displayTitle = String(groupTitle || "").replace(
          /\s*→\s*/g,
          " -> ",
        );
